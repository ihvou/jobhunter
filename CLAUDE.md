...
- Regular agents stay on the `openai-codex` app-server with read-only sandbox. Engineer is the only agent that selects `codex-cli/gpt-5.5`; the shared `agents.defaults.cliBackends.codex-cli` entry is intentionally wired to Engineer's `/workspace` and Engineer `CODEX_HOME`. The OpenAI plugin owns `codex-cli` bundled MCP/native-tool registration, so user config overrides command/env/args only.
