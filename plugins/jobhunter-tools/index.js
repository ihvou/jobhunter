...
export default definePluginEntry({
  ...
  register(api) {
    ...
    // DISABLED 2026-05-26: Find recruiters / Find LinkedIn buttons hidden pending
    // Path B implementation (LinkedIn via dedicated research-account cookies +
    // Playwright). Free Firecrawl-via-Google-LinkedIn-index path had ~30% hit
    // rate on jobs (10-job live test) — too low for production. Service
    // endpoint /jobs/find_recruiters and underlying helpers remain in
    // service.py / database.py / firecrawl.py as dormant infrastructure;
    // re-enable by uncommenting this register() block, restoring the [Find
    // recruiters] button + callback handling in the jobhunter_get_more_jobs
    // description, re-adding to openclaw.plugin.json contracts.tools, and
    // updating tests/index.test.js expectedToolNames.
    //
    // register(api, {
    ...
    register(api, {
      name: "leadhunter_rescore_leads",
      label: "Leadhunter Rescore Leads",
      description:
        "Re-evaluate confidence and why_match for saved leads against the CURRENT Leadhunter ICP (input/icp.local.md). " +
        "Use this when the user asks: 'rescore my leads', 'rescore leads', 'update lead scores', 're-evaluate leads', " +
    ...
    // DISABLED 2026-05-26: see jobhunter_find_recruiters above for context.
    // Lead-side Firecrawl founder lookup hidden pending Path B. Service
    // endpoint /leads/find_linkedin and helpers remain dormant.
    //
    // register(api, {
