...
4. Apply only reversible audited changes when evidence is concrete:
   `jobhunter_apply_directive_edit`, `jobhunter_apply_icp_edit`,
   `jobhunter_set_source_priority`, `jobhunter_set_source_status`.
5. File tasks for other agents with `jobhunter_file_task`.
6. Use `jobhunter_propose_actions` for user-gated scoring, source-addition,
   profile rewrite, or bulk archive changes.
7. Write `jobhunter_write_status_report`.
8. Send exactly one Telegram stakeholder report, then stop.
